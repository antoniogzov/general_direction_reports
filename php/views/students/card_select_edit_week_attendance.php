...
if ($submodule == "attendance_report") {
    if (($grants & 8)) {
        ...
        (SELECT groups.id_group, groups.group_code, assg.print_school_report_card, assg.assignment_active, rel_coord_aca.no_teacher, groups.group_type_id, sbj.name_subject
        ...
        SELECT gps.id_group, gps.group_code, asgm.print_school_report_card, asgm.assignment_active, rel_coord_aca.no_teacher, gps.group_type_id, sbj.name_subject
        FROM iteach_academic.relationship_coordinators_academic_areas AS rel_coord_aca
    ...
    if (($grants & 8)) {
        ...
        (SELECT sbj.id_subject, sbj.name_subject, groups.id_group, groups.group_code, assg.print_school_report_card, assg.assignment_active, rel_coord_aca.no_teacher, groups.group_type_id
        FROM iteach_academic.relationship_coordinators_academic_areas AS rel_coord_aca
        INNER JOIN school_control_ykt.level_combinations AS lvl_com ON rel_coord_aca.id_level_combination = lvl_com.id_level_combination
        INNER JOIN school_control_ykt.groups AS groups ON lvl_com.id_campus = groups.id_campus AND lvl_com.id_section = groups.id_section
        INNER JOIN school_control_ykt.academic_levels_grade AS acdlvldg ON lvl_com.id_academic_level  = acdlvldg.id_academic_level AND groups.id_level_grade = acdlvldg.id_level_grade
        ...
        SELECT sbj.id_subject, sbj.name_subject, gps.id_group, gps.group_code, asgm.print_school_report_card, asgm.assignment_active, rel_coord_aca.no_teacher, gps.group_type_id
        FROM iteach_academic.relationship_coordinators_academic_areas AS rel_coord_aca
        INNER JOIN school_control_ykt.assignments AS asgm ON rel_coord_aca.coordinators_group_id = asgm.coordinators_group_id
        INNER JOIN school_control_ykt.subjects AS sbj ON asgm.id_subject = sbj.id_subject 
        INNER JOIN school_control_ykt.groups AS gps ON asgm.id_group = gps.id_group 
        ...
        SELECT sbj.id_subject, sbj.name_subject, groups.id_group, groups.group_code, asg.print_school_report_card, asg.assignment_active, asg.no_teacher, groups.group_type_id
        FROM school_control_ykt.assignments AS asg 
        INNER JOIN school_control_ykt.subjects AS sbj ON asg.id_subject = sbj.id_subject
        INNER JOIN school_control_ykt.groups AS groups ON asg.id_group = groups.id_group)

        AS u
        WHERE no_teacher = $no_teacher AND id_group = $id_group AND print_school_report_card = 1 AND assignment_active = 1 AND group_type_id != 4";
        ...
        WHERE asg.no_teacher = '$no_teacher' AND gps.id_group = '$id_group' AND asg.show_list_teacher = '1'";
        //--- --- ---//
        $stmt_groups = "SELECT DISTINCT groups.id_group, groups.group_code
        FROM school_control_ykt.assignments AS asg
        INNER JOIN school_control_ykt.subjects AS sbj ON asg.id_subject = sbj.id_subject
...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <div class="row">
            <!-- -->
            <div class="col-md-3">
                <?php if (isset($_GET['class_block'])) : ?>
                    <?php if ($_GET['class_block'] == 2) : ?>
                        ...
                        <div class="custom-control custom-radio">
                            <input type="radio" checked id="customRadio2" value="2" name="class_block" class="custom-control-input">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    ?>
                        ...
                        <input class="form-control" id="week_picker" placeholder="Elegir semana..." type="text" value="<?= date('n') ?>/<?= date('j') ?>/<?= date('Y') ?>">
                    <?php
                    } else {

                    ?>
                        <label class="form-control-label" for="exampleDatepicker">* Seleccione una semana</label>
                        <input class="form-control" id="week_picker" placeholder="Elegir semana..." value="<?= date('n') ?>/<?= date('j') ?>/<?= date('Y') ?>" type="text">
                    <?php
