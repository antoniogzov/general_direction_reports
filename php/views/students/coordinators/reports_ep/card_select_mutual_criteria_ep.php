...
if (isset($_GET['id_academic_area']) && isset($_GET['id_academic_level']) && isset($_GET['criteria']) && isset($_GET['id_period']) && isset($_GET['id_level_grade'])) {
    ...
    $criteria = mb_strtoupper(str_replace('_', ' ', $criteria));
    ...
    $getMutualCriteria = $expected_learnings->getMutualCriteriaEP($id_academic_area, $id_academic_level, $no_teacher, $id_period, $id_level_grade);
...
<div class="card mb-4">
    ...
    <div class="card-header">
        <h3 class="mb-0">Criterios Comúnes de Plan de Evaluación</h3>
    </div>
    <!-- Card body -->
    <div class="card-body">
                            ...
                            <?php if (!empty($getMutualCriteria)) : ?>
                                <?php foreach ($getMutualCriteria as $mutual_criteria) : ?>
                                    <?php if (isset($criteria)) : ?>
                                        <?php if ($criteria == $mutual_criteria->id_evaluation_source) : ?>
                                            <option selected value="<?= $mutual_criteria->id_evaluation_source ?>"><?= mb_strtoupper($mutual_criteria->evaluation_name) ?></option>
                                        <?php else : ?>
                                            <option value="<?= $mutual_criteria->id_evaluation_source ?>"><?= mb_strtoupper($mutual_criteria->evaluation_name) ?></option>
                                        <?php endif; ?>
                                    <?php else : ?>
                                        <option value="<?= $mutual_criteria->id_evaluation_source ?>"><?= mb_strtoupper($mutual_criteria->evaluation_name) ?></option>
                                    <?php endif; ?>
...
<?php
if (isset($_GET['id_academic_area']) && isset($_GET['id_academic_level']) && isset($_GET['criteria']) && isset($_GET['id_period']) && isset($_GET['id_level_grade'])) {
    include_once 'show_mutual_criteria_reports_ep.php';
}
?>
