                                ...
                                <?php foreach ($getPeriods as $periods) : ?>
                                    <?php if (isset($id_period)) : ?>
                                        <?php if ($id_period == $periods->id_period_calendar) : ?>
                                            <option data-no-period="<?= $periods->no_period?>"selected value="<?= $periods->id_period_calendar ?>"><?= mb_strtoupper($periods->no_period) ?> <?=$section_descr = isset($periods->section_descr) ? $periods->section_descr : '';  ?> </option>
                                        <?php else : ?>
                                            <option  data-no-period="<?= $periods->no_period?>" value="<?= $periods->id_period_calendar ?>"><?= mb_strtoupper($periods->no_period) ?> <?=$section_descr = isset($periods->section_descr) ? $periods->section_descr : '';  ?> </option>
                                        <?php endif; ?>
                                    <?php else : ?>
                                        <option  data-no-period="<?= $periods->no_period?>" value="<?= $periods->id_period_calendar ?>"><?= mb_strtoupper($periods->no_period) ?> <?=$section_descr = isset($periods->section_descr) ? $periods->section_descr : '';  ?> </option>
                                    <?php endif; ?>
...
<?php
if (isset($_GET['id_academic_area']) && isset($_GET['id_academic_level']) && isset($_GET['id_period']) && isset($_GET['no_period']) && isset($_GET['id_level_grade'])) {
    include_once 'show_programmatic_advance.php';
}
?>
