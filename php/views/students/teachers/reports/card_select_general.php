...
if (isset($_GET['week'])&&isset($_GET['id_academic'])) {
    $val_week = $_GET['week'];
...
if (($grants & 8)) {
    ...
    (SELECT sbj.id_academic_area, aca.name_academic_area, rel_coord_aca.no_teacher, assg.print_school_report_card, assg.assignment_active
    ...
    INNER JOIN school_control_ykt.subjects AS sbj ON assg.id_subject = sbj.id_subject AND lvl_com.id_academic_area = 
    sbj.id_academic_area
    INNER JOIN school_control_ykt.academic_areas AS aca ON sbj.id_academic_area = aca.id_academic_area
    
    UNION 

    SELECT sbj.id_academic_area, aca.name_academic_area, rel_coord_aca.no_teacher, asgm.print_school_report_card, asgm.assignment_active
    FROM iteach_academic.relationship_coordinators_academic_areas AS rel_coord_aca
    ...
    (SELECT assg.print_school_report_card, assg.assignment_active, CONCAT(col.nombres_colaborador,' ', col.apellido_paterno_colaborador, ' ', col.apellido_materno_colaborador) AS teacher_name, col.no_colaborador, rel_coord_aca.no_teacher, sbj.id_academic_area
    FROM iteach_academic.relationship_coordinators_academic_areas AS rel_coord_aca
    INNER JOIN school_control_ykt.level_combinations AS lvl_com ON rel_coord_aca.id_level_combination = lvl_com.id_level_combination
    INNER JOIN school_control_ykt.groups AS groups ON lvl_com.id_campus = groups.id_campus AND lvl_com.id_section = groups.id_section
    INNER JOIN school_control_ykt.academic_levels_grade AS acdlvldg ON lvl_com.id_academic_level  = acdlvldg.id_academic_level AND groups.id_level_grade = acdlvldg.id_level_grade
    ...
    SELECT asgm.print_school_report_card, asgm.assignment_active, CONCAT(col.nombres_colaborador,' ', col.apellido_paterno_colaborador, ' ', col.apellido_materno_colaborador) AS teacher_name, col.no_colaborador, rel_coord_aca.no_teacher, sbj.id_academic_area
    FROM iteach_academic.relationship_coordinators_academic_areas AS rel_coord_aca
    INNER JOIN school_control_ykt.assignments AS asgm ON rel_coord_aca.coordinators_group_id = asgm.coordinators_group_id
    INNER JOIN school_control_ykt.subjects AS sbj ON asgm.id_subject = sbj.id_subject 
    INNER JOIN school_control_ykt.groups AS gps ON asgm.id_group = gps.id_group 
    ...
    WHERE no_teacher = $_SESSION[colab] AND id_academic_area = $id_academic AND print_school_report_card = 1 AND assignment_active = 1
    ...
    WHERE no_teacher = $no_teacher AND id_group = $id_group AND id_academic_area = $id_academic_area AND print_school_report_card = 1 AND assignment_active = 1
    LIMIT 1";
    /* } */
} /* else if ($grants & 4) {
        //--- --- ---//
...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="form-group" style="display:<?= $display ?>" id="div_week">
