...
if (($grants & 8)) {
    ...
    $stmt_section = "
    SELECT u.id_academic_area, u.name_academic_area FROM 
    ...
    /*if (!empty($GetAssignment)) {
        ...
        //--- --- ---//
            foreach ($getLevel as $level) {
                $id_level_combination = $level->id_level_combination;
            }
...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <div class="row">
            ...
            <?php if($grants & 8): ?>
            <div class="col-md-4">
              <div class="custom-control custom-radio mb-3 form-check-inline" style="display: unset !important;">
                <input name="radio-averages-type" class="custom-control-input" id="customRadio5" value="normal" type="radio" <?= $average_type == 'normal' ? 'checked=""' : '' ?>>
                <label class="custom-control-label" for="customRadio5">Promedios normales</label>
              </div>
              <div class="custom-control custom-radio mb-3 form-check-inline" style="display: unset !important;">
                <input name="radio-averages-type" class="custom-control-input" id="customRadio6" value="dinamyc" type="radio" <?= $average_type == 'dinamyc' ? 'checked=""' : '' ?>>
                <label class="custom-control-label" for="customRadio6">Promedios dinámicos</label>
              </div>
            </div>
            <?php endif; ?>
            <!-- -->
