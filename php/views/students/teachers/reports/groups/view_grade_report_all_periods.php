						...
						<table class="table table-hover table-sm" border="1" id="<?= $group->group_code ?>">
							<thead>
								<tr class="sticky-header">
									...
									<th rowspan="2" class="align-middle" data-toggle="tooltip" data-placement="top" title="Promedio general redondeado" style="font-weight: bold !important; color: black !important;">P G R</th>
							...
							<tbody style="color: black !important;">
								<?php foreach ($listStudent as $student) :
									if ($average_type == 'normal') {
										$final_all_grade_rounded = $student->averages->final_all_grade_rounded;
										$final_all_grade = $student->averages->final_all_grade;
									} else if ($average_type == 'dinamyc') {
										$final_all_grade_rounded = $student->averages->final_all_grade_rounded_dynamic;
										$final_all_grade = $student->averages->final_all_grade_dynamic;
									}
								?>
									<tr id="<?= $student->id_student ?>">
										...
										<td class="text-center"><?= $final_all_grade_rounded ?></td>
										<td class="text-center"><?= $final_all_grade ?></td>

										<?php foreach ($student->averages->assgs as $std_assg) :
											//final_grade_periods_dynamic
											if ($average_type == 'normal') {
												$final_grade_periods = $std_assg->final_grade_periods;
											} else if ($average_type == 'dinamyc') {
												$final_grade_periods = $std_assg->final_grade_periods_dynamic;
											}

										?>
											<td style="border-left: 3.2px double black; font-weight: bold; color: black; font-size: small; background-color: <?= getBCGCByAverage($std_assg->final_grade_periods); ?>" class="text-center"><?= $final_grade_periods ?></td>
											...
											<?php foreach ($std_assg->periods as $assg_periods) :
												...
												if ($average_type == 'normal') {
													$average_view = $assg_periods->average;
													$average_princ = $assg_periods->average;
													$average_sec = $assg_periods->grade_period_calc;
												} else if ($average_type == 'dinamyc') {
													$average_view = $assg_periods->grade_period_calc;
													$average_princ = $assg_periods->average;
													$average_sec = $assg_periods->grade_period_calc;
												}
											?>
												<td id="tdGP<?= $assg_periods->id_grade_period ?>" style="background-color: <?= getBCGCByAverage($assg_periods->average); ?>" class="text-center" onclick="getCriteriaDetails('<?= $assg_periods->id_grade_period ?>', '<?= $student->id_student ?>','<?= $std_assg->id_assignment ?>','<?= $average_princ ?>','<?= $average_sec ?>')"><?= $average_view ?></td>
											<?php
												$average_princ = '';
												$average_sec = '';
											endforeach; ?>
