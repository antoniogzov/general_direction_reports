...
$(document).on("change", "#id_academic__level", function () {
  ...
  id_academic_area = $("#id_academic_area").val();
  getGroups(id_academic_level, id_academic_area);
});
$(document).on("change", "#id_group", function () {
  var id_academic_level = $("#id_academic__level").val();
  id_academic_area = $("#id_academic_area").val();
  getPeriods(id_academic_level, id_academic_area);
});
$(document).on("change", "#id_period", function () {
  ...
  if (urlParams.has("submodule")) {
    ...
    const id_academic_area = $("#id_academic_area").val();
    const no_period = $("#id_period option:selected").attr("data-no-period");
    const id_group = $("#id_group option:selected").val();
    const submodule = urlParams.get("submodule");
    const id_period = $(this).val();
    window.location.search =
      "submodule=" +
      submodule +
      "&id_academic_level=" +
...
function getGroups(id_academic_level, id_academic_area) {
  //--- --- ---//
  loading();
  $("#id_group").html("");
  //--- --- ---//
  $.ajax({
    ...
    method: "POST",
    data: {
      mod: "getGroupsByIdAcademicLevelAndAcademicArea",
...
function getPeriods(id_academic_level, id_academic_area) {
  //--- --- ---//
  loading();
  //--- --- ---//
  $.ajax({
    ...
    .done(function (data) {
      ...
      if (data.response) {
        ...
        //--- --- ---//
        $("#id_period").html(options_reports);
        //--- --- ---//
        swal.close();
        //--- --- ---//
      } else {
...
$(document).on("click", ".trDetailsAttend", function () {
  loading();
  ...
  getAssistanceDetailsReports(start_date, end_date, id_assignment, id_student);
});
function getAssistanceDetailsReports(
  ...
  $.ajax({
    ...
    method: "POST",
    data: {
      mod: "getAttendanceReportDetailsAll",
