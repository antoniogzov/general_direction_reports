function downloadReportPrimaryBangueoloHebrew(data, installment) {
  //console.log(data);
  var table1 = '';
  var table = '';
  //--- --- ---//
  ...
  doc.addFont('js/vendor/jsPDF/fonts/VarelaRound-Regular.ttf', 'VarelaRound-Regular', 'normal');
  //--- --- ---//
  doc.setFontSize(11);
...
function downloadReportPrimaryBangueoloSpanish(data, installment) {
  //console.log(data);
  var table1 = "";
  var table = "";
  //--- --- ---//
  ...
  doc.autoTable({
    ...
    head: [
      ...
      [
        {
          content: "MAPAS DE APRENDIZAJE: EVALUACIÓN DE ESPAÑOL",
          styles: { halign: "center" },
        },
      ],
    ],
  ...
  //--- MDA INGLES ---//
  var positionYfinal_Eng = doc.lastAutoTable.finalY + 15;
  doc.autoTable({
    theme: "plain",
    startY: positionYfinal_Eng,
    tableWidth: 190,
    margin: {
      left: 12,
    },
    ...
    head: [
      [
        {
          content: "MAPAS DE APRENDIZAJE: EVALUACIÓN DE INGLÉS",
          styles: { halign: "center" },
        },
      ],
    ],
  });
  positionYfinal_Eng = doc.lastAutoTable.finalY + 15;
  if (data.results_evc_normal_eng.length > 0) {
    for (var i = 0; i < data.results_evc_normal_eng.length; i++) {
      if ((data.results_evc_normal_eng[i].topic.id_learning_map = "21")) {
        //--- --- --- ---//
        var catalog_symbol = [];
        for (
          var b = 0;
          b <
          data.results_evc_normal_eng[i].data.questions_evaluations.evaluations
            .length;
          b++
        ) {
          catalog_symbol.push(
        ...
        var header = [
          [
            ...
            {
              content:
                data.results_evc_normal_eng[i].data.assgs[0].assg.teacher_name,
              colSpan:
                data.results_evc_normal_eng[i].data.questions_evaluations
                  .evaluations.length,
              styles: {
        ...
        for (
          var f = 0;
          f <
          data.results_evc_normal_eng[i].data.questions_evaluations.questions
            .length;
          f++
        ) {
          //console.log('Pregunta');
          ...
          for (
            ...
            for (
              var d = 0;
              d < data.results_evc_normal_eng[i].data.assgs[0].answers.length;
              d++
            ) {
              //--- --- ---//
              if (
                data.results_evc_normal_eng[i].data.assgs[0].assg
                  .ascc_lm_assgn ==
                  data.results_evc_normal_eng[i].data.assgs[0].answers[d]
                    .ascc_lm_assgn &&
        ...
        doc.text(
          23,
          positionYfinal_Eng - 3,
          data.results_evc_normal_eng[i].topic.name_question_group.toUpperCase()
        );
        //--- --- ---//
        doc.autoTable({
          startY: positionYfinal_Eng,
  ...
  if (data.final_comments_evc_normal_eng.length > 0) {
    if (data.final_comments_evc_normal_eng[0].comments.length > 0) {
      ...
      //--- --- ---//
      //--- --- ---//
      positionYfinal_Eng += 5;
      doc.text(23, positionYfinal_Eng - 3, "COMENTARIOS");
      //--- --- ---//
      doc.autoTable({
        startY: positionYfinal_Eng,
        margin: {
...
function downloadReportPreparatoryBangueoloMDAhebrew(data, installment) {
  if (data.results_evc_normal.length > 0 || data.results_evc_mejanejet.length > 0) {
      //--- --- ---//
      var imgLogo = new Image();
      imgLogo.src = 'images/logos/sec_prepa_mujeres_bangueolo_png.png';
      imgLogo.onload = function() {
          ...
          doc.addFileToVFS("js/vendor/jsPDF/fonts/VarelaRound-Regular-normal.ttf", getFontVarelaRegular());
          doc.addFont('js/vendor/jsPDF/fonts/VarelaRound-Regular-normal.ttf', 'VarelaRound-Regular', 'normal');
          doc.addFileToVFS('js/vendor/jsPDF/fonts/VarelaRound-Regular.ttf');
          //--- TABLA DE ENCABEZADO ---//
          doc.setFontSize(11);
          doc.autoTable({
              ...
              head: [
                  ...
                  [{
                      content: ("Alumno: " + data.students.student_code + " | " + data.students.student_name).toUpperCase(),
                      styles: {
                          fontSize: 11,
                          halign: "center"
                      },
          ...
          doc.addImage(imgLogo, "png", 15, 11, 25, 24);

          //--- MDA GENERAL ---//
          var positionYfinal = 48;
          if (data.results_evc_normal.length > 0) {
              for (var i = 0; i < data.results_evc_normal.length; i++) {
                  for (var a = 0; a < data.results_evc_normal[i].data.assgs.length; a++) {
                      ...
                      for (var b = 0; b < data.results_evc_normal[i].data.questions_evaluations.evaluations.length; b++) {
                          catalog_symbol.push(data.results_evc_normal[i].data.questions_evaluations.evaluations[b].symbol);
                      }
                      //--- --- ---//
                      var header = [
                          [{
                              content: (data.results_evc_normal[i].data.assgs[a].assg.name_subject).toUpperCase(),
                              rowSpan: 2,
                              styles: {
                                  halign: "center",
                                  valign: 'middle'
                              ...
                              content: data.results_evc_normal[i].data.assgs[a].assg.teacher_name,
                              colSpan: data.results_evc_normal[i].data.questions_evaluations.evaluations.length,
                              styles: {
                                  halign: "center",
                              },
                          }, ], catalog_symbol
                      ...
                      //--- LISTA PREGUNTAS ---//
                      let body_mda = [];
                      for (let f = 0; f < data.results_evc_normal[i].data.questions_evaluations.questions.length; f++) {
                          //--- --- ---//
                          let data_question = [data.results_evc_normal[i].data.questions_evaluations.questions[f].question];
                          let answer_to_question = false;
                          //--- --- ---//
                          for (let b = 0; b < data.results_evc_normal[i].data.questions_evaluations.evaluations.length; b++) {
                              //--- --- ---/
                              let answer = null;
                              //--- RESPUESTAS ---//
                              for (let d = 0; d < data.results_evc_normal[i].data.assgs[a].answers.length; d++) {
                                  //--- --- ---//
                                  if (data.results_evc_normal[i].data.assgs[a].assg.ascc_lm_assgn == data.results_evc_normal[i].data.assgs[a].answers[d].ascc_lm_assgn && data.results_evc_normal[i].topic.assc_mpa_id == data.results_evc_normal[i].data.assgs[a].answers[d].assc_mpa_id && data.results_evc_normal[i].data.questions_evaluations.questions[f].id_question_bank == data.results_evc_normal[i].data.assgs[a].answers[d].id_question_bank && data.results_evc_normal[i].data.questions_evaluations.evaluations[b].id_evaluation_bank == data.results_evc_normal[i].data.assgs[a].answers[d].id_evaluation_bank) {
                                      ...
                                      answer = {
                                          content: "",
                                          styles: {
                                              fillColor: data.results_evc_normal[i].data.questions_evaluations.evaluations[b].colorHTML
                                          }
                                      };
                                  }
                              }
                      ...
                      for (var d = 0; d < data.final_comments_evc_normal.length; d++) {
                          for (var x = 0; x < data.final_comments_evc_normal[d].comments.length; x++) {
                              if (data.final_comments_evc_normal[d].comments[x].ascc_lm_assgn == data.final_comments_evc_normal[d].assg.ascc_lm_assgn && data.results_evc_normal[i].data.assgs[a].assg.ascc_lm_assgn == data.final_comments_evc_normal[d].comments[x].ascc_lm_assgn) {
                                  //--- --- ---//
                                  let comment = {
                                      content: fixLetter(data.final_comments_evc_normal[d].comments[x].comments1),
                                      colSpan: (data.results_evc_normal[i].data.questions_evaluations.evaluations.length + 1),
                                      styles: {
                                          fontStyle: 'bold',
                                          fontSize: 8
                                      }
                                  };
                                  body_mda.push([comment]);
                                  //--- --- ---//
                              }
                          }
                      }
                      ...
                      //--- --- ---//
                      positionYfinal = doc.lastAutoTable.finalY + 6;
                      //--- --- ---//
                  }
                  //--- --- ---//
              }
                  ...
                  for (var i = 0; i < data.results_evc_mejanejet.length; i++) {
                      ...
                      for (var a = 0; a < data.results_evc_mejanejet[i].data.assgs.length; a++) {
                          ...
                          for (var b = 0; b < data.results_evc_mejanejet[i].data.questions_evaluations.evaluations.length; b++) {
                              catalog_symbol.push(data.results_evc_mejanejet[i].data.questions_evaluations.evaluations[b].symbol);
                          }
                          //--- --- ---//
                          var header = [
                              [{
                                  content: (data.results_evc_mejanejet[i].data.assgs[a].assg.name_subject).toUpperCase(),
                                  rowSpan: 2,
                                  styles: {
                                      halign: "center",
                                      valign: 'middle'
                                  ...
                                  content: data.results_evc_mejanejet[i].data.assgs[a].assg.teacher_name,
                                  colSpan: data.results_evc_mejanejet[i].data.questions_evaluations.evaluations.length,
                                  styles: {
                                      halign: "center",
                                  },
                              }, ], catalog_symbol
                          ...
                          for (var f = 0; f < data.results_evc_mejanejet[i].data.questions_evaluations.questions.length; f++) {
                              //--- --- ---//
                              let data_question = [data.results_evc_mejanejet[i].data.questions_evaluations.questions[f].question];
                              let answer_to_question = false;
                              //--- --- ---/
                              for (var b = 0; b < data.results_evc_mejanejet[i].data.questions_evaluations.evaluations.length; b++) {
                                  //--- --- ---/
                              ...
                              for (var d = 0; d < data.final_comments_mejanejet.length; d++) {
                                  for (var x = 0; x < data.final_comments_mejanejet[d].comments.length; x++) {
                                      if (data.final_comments_mejanejet[d].comments[x].ascc_lm_assgn == data.final_comments_mejanejet[d].assg.ascc_lm_assgn && data.results_evc_mejanejet[i].data.assgs[a].assg.ascc_lm_assgn == data.final_comments_mejanejet[d].comments[x].ascc_lm_assgn) {
                                          //--- --- ---//
                                          let comment = {
                                              content: fixLetter(data.final_comments_mejanejet[d].comments[x].comments1),
                                              colSpan: (data.results_evc_mejanejet[i].data.questions_evaluations.evaluations.length + 1),
                                              styles: {
                                                  fontStyle: 'bold',
                                                  fontSize: 8
                                              }
              ...
              doc.setFontSize(7);
              let date = new Date();
              let date_issue = String(date.getDate()).padStart(2, "0") + "/" + String(date.getMonth() + 1).padStart(2, "0") + "/" + date.getFullYear();

              const pages = doc.internal.getNumberOfPages();

              for (let j = 1; j < pages + 1; j++) {
                  doc.setPage(j);
                  doc.text(92, 272, "Código de alumno: " + data.students.student_code + "  |  Grupo iTeach: " + data.students.group_code + "  |  Fecha de emisión: " + date_issue);
                  doc.text(20, 272, 'pág ' + j);
              }

              doc.save(data.students.student_code.toUpperCase() + ' - ' + data.students.student_name + ".pdf");
...
function downloadReportPreparatoryBangueoloMDAspanish(data, installment) {
  //--- MDA GENERAL ---//
  if (data.results_evc_normal.length > 0) {
      //--- --- ---//
      var imgLogo = new Image();
...
function downloadFinalCommentsReport(data, installment) {
  console.log(data);
  var table1 = '';
  var table = '';
  //--- --- ---//
